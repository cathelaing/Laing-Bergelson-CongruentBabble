...
CPdata$C1Obj <- as.character(CPdata$C1Obj)
CPdata$C1Prompt <- as.character(CPdata$C1Prompt)

CPdata_noC1match <- CPdata %>%
  mutate(match = C1Obj == C1Prompt,                                       # mark as T all instances where C1Obj matches C1Prompt
         match = ifelse(C1Obj == "none" & C1Prompt == "none", F, match)) %>%  # keep instances where both C1Obj and C1Prompt are 'none'
  filter(match == F)


Prod.Prompt <- CPdata_noC1match %>%
  filter(!grepl("^\\[",cgprompt)) %>%     # exclude all instances where object is coded as [toy], [unclear], etc but keep insances of e.g. 'lxx [name]'
  group_by(subj, Prod.Prompt.Cong, month, VMSgroup) %>%
  tally() %>%
...
Prod.Obj <- CPdata_noC1match %>%                  # Only includes 43 infants as subj 944 doesn't have any attended objects
...
# Join all dfs with ProdData_noC1match

ProdData_noC1match <- Prod.Prompt %>%          # Does infant's CP match the attended object?
  left_join(Prod.Obj) %>%       # Does infant's CP match the caregiver prompt
...
# Create dataframes for analysis of inREP vs outREP consonants

# Prompt

matchPrompt <- ProdData_noC1match %>% 
  dplyr::select(subj, VMSgroup, PCVMSMatch.Prompt) %>% 
  mutate(Type = "inREP") %>% 
  rename(PC = PCVMSMatch.Prompt)

nomatchPrompt <- ProdData_noC1match %>% 
