...
Figure1 <- ggplot(vmscount, aes(x= reorder(month_chron, n), y=n, alpha=inREP, fill = Consonant, colour = Consonant))  +  
  geom_col(lwd = 1, aes(linetype = VMSgroup)) +
  scale_linetype_manual(values = c("withVMS" = "solid", "noVMS" = "twodash"), guide = F) +
  scale_alpha_discrete(range=c(0.1,0.6), 
                    labels = c("outREP", "inREP")) +
  facet_grid(~subj, scales = "free_x", switch = "x") + 
  xlab("Subject") + 
  ylab("n tokens at 0;10 (n=44) and 0;11 (n=17)") +
  scale_colour_discrete(breaks = c("p", "t", "k", "m", "n"), guide = F) +
  scale_fill_discrete(breaks = c("p", "t", "k", "m", "n")) +
  scale_y_continuous(breaks = c(0, 20, 50, 200, 400, 600)) +
  ...
  guides(VMSgroup = FALSE) +
  labs(alpha = "VMS") +
  geom_hline(yintercept = 20, linetype = 2, lwd = .8, colour = "gray27") +
  geom_hline(yintercept = 50, linetype = 3, lwd = .8, colour = "gray27") 
